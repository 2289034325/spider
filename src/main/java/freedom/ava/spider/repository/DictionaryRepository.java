...
public interface DictionaryRepository {
    ...
    @Select("select * from word where lang=#{lang} and spell=#{spell} and pronounce=#{pronounce} and deleted=0")
    List<Word> selectWordsBySpellAndPronounce(@Param("lang") Integer lang, @Param("spell") String spell, @Param("pronounce") String pronounce);

    @Select("select * from word where lang=#{lang} and (pronounce=#{pronounce} or forms like '%' #{form} '%') and deleted=0")
    List<Word> selectWordsByPronounceOrForm(@Param("lang") Integer lang, @Param("pronounce") String pronounce, @Param("form") String form);
    ...
    Integer selectDefaultBook(@Param("user_id") int user_id, @Param("lang") int lang);
